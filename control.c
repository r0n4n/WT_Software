...
//    state->il.abc.a = -(state->il.abc.c + state->il.abc.b) ; // get the last current line
...
//       reference_voltage_saturation(state, us) ; 
...
void reference_voltage_saturation(state *state, signal *us){
    ...
    us_complex.module = module_square(us_complex) ; // d�+q�
    us_complex.angle = get_complex_angle(us_complex) ; 
    
    int vout_by_two_square = state->vout*state->vout/4 ; // (Vdc/2)�

    us_complex.module = saturation(-vout_by_two_square, vout_by_two_square, us_complex.module) ;    
    complex_real_imag(&us_complex) ; 
    us->dq.d  = us_complex.real ; 
    us->dq.q = us_complex.imag ; 
    int i = 1 ; 
}
